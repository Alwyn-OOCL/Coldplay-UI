...
export default function ConcertItem({ concert }) {
  ...
  return (
      <Link to={`/concert/${concert.concertId}`} className="concert-item">
        <img src={concert.concertImage} alt={concert.concertName} className="concert-image" />
        <div className="concert-details">
          <h2>{concert.concertName}</h2>
          <p><strong>Date:</strong> {dateString}</p>
          <p><strong>Time:</strong> {timeString}</p>
          <p><strong>Venue:</strong> {concert.venueName}</p>
          <p><strong>Location:</strong> {concert.venueCity}, {concert.venueCountry}</p>
