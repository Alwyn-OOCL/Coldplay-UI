...
const BookingForm = ({ formData, handleChange, handleNext, concert }) => {
  ...
  const validateForm = () => {
    ...
    formData.areaPreferences.forEach((preference, index) => {
      if (!preference) {
        newErrors[`areaPreference${index}`] =
          `Priority ${index + 1} is required`;
      }
    });
    formData.audienceDetails.forEach((audience, index) => {
      if (!audience.name) {
        newErrors[`audienceName${index}`] =
          `Name of audience ${index + 1} is required`;
      }
      if (!audience.idCard) {
        newErrors[`audienceIdCard${index}`] =
          `ID card number of audience ${index + 1} is required`;
      }
    });
    setErrors(newErrors);
